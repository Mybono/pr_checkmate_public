...
`ok: summary.failed === 0` — *any* check returning `fail`, in *any* phase, fails it. Several
...
execution order, and how an *unexpected exception* is graded.
...
In a pull request PR CheckMate receives a base and head SHA and most checks examine **only the
changed files**. Outside a PR context there is no diff range and those checks fall back to a
full-project scan. Checks that only make sense on a diff (`Banned Imports`, `Custom Rules`, the
git-diff family) skip entirely when no range is available.
...
- **`ignoreDirs`** *replaces* the default ignore list rather than extending it — the generated
...
- **`<check>.ignore` filters finding labels, not file paths — except for [YAML Lint](quality/yaml-lint.md).**
  ...
  **case-insensitive substring of the finding's label**. So `"ignore": ["md5"]` mutes the
  `MD5 (weak hash …)` finding everywhere, and does *not* mean "skip files named md5". `yamlLint.ignore`
